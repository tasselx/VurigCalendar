...
//    if (hasPreviousDays) {
//        //        animationView_B.frameY = animationView_A.frameY - (animationView_B.frameHeight-kVRGCalendarViewDayHeight) + 3;
//        animationView_B.frameX =  (animationView_A.frameWidth - kVRGCalendarViewDayWidth) +3;
//    } else {
//        //        animationView_B.frameY = animationView_A.frameY - animationView_B.frameHeight + 3;
...
//                         if (hasPreviousDays) {
//                             //                             animationView_A.frameY = animationView_B.frameHeight-(kVRGCalendarViewDayHeight+3);
//                             animationView_A.frameX = animationView_B.frameWidth -(kVRGCalendarViewWidth +3);
//                             
//                         } else {
//                             //                             animationView_A.frameY = animationView_B.frameHeight-3;
...
- (void)drawRect:(CGRect)rect
{
    ...
    [delegate currentMonth:self.currentMonth];
...
//    
   ...
   // [weekdays moveObjectFromIndex:0 toIndex:6];
    ...
    CGContextSetFillColorWithColor(context, 
                                   [UIColor colorWithString:@"999999"].CGColor);
    for (int i =0; i<[weekdays count]; i++) {
        NSString* orgValue = [weekdays objectAtIndex:i];
        NSString *weekdayValue = (NSString *)[orgValue substringWithRange:NSMakeRange(1, 1) ];
        UIFont *font = [UIFont fontWithName:@"HelveticaNeue" size:12];
        [weekdayValue drawInRect:CGRectMake(i*(kVRGCalendarViewDayWidth+2), 0, kVRGCalendarViewDayWidth+2, 20) withFont:font lineBreakMode:NSLineBreakByClipping alignment:NSTextAlignmentCenter];
    }
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (i<firstWeekDay) { //previous month
            ...
            NSString *hex = (isSelectedDatePreviousMonth) ? @"666666" : @"999999";
            ...
            NSString *hex = (isSelectedDateNextMonth) ? @"666666" : @"999999";
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        } else { //current month
            // isCurrentMonth = YES;
            targetDate = (i-firstWeekDay)+1;
            NSString *hex = (isSelectedDatePreviousMonth || isSelectedDateNextMonth) ? @"999999" : @"666666";
            CGContextSetFillColorWithColor(context, 
                                           [UIColor colorWithHexString:hex].CGColor);
        }
...
//            if (todayBlock !=i) {
////                CGContextSetFillColorWithColor(context, [UIColor colorWithRed:0./255 green:138./255 blue:255./255 alpha:1].CGColor);
...
//                
//                CGContextSetFillColorWithColor(context,
//                                               [UIColor colorWithRed:0/255.0 green:138/255.0 blue:255/255.0 alpha:1].CGColor);
//                UIImage *image=[UIImage imageNamed:@"schedule_ico_circle"];
//                [image drawInRect:CGRectMake(targetX, targetY+10, image.size.width, image.size.height)];
//            }
...
//            UIImage *image=[UIImage imageNamed:@"schedule_ico_circle"];
//            [image drawInRect:rectangleGrid];
    ...
    for (int i = 0; i<[self.markedDates count]; i++) {
       ...
       // CGRect rectangle = CGRectMake(targetX+14,targetY,5,5);
        //CGContextAddRect(context, rectangle);
        CGContextSetFillColorWithColor(context, [UIColor blackColor].CGColor);
        CGContextAddArc(context,targetX+16, targetY, 2, 0, 2*M_PI, 0); //添加一个圆
